...
function stagingAdmin(){return location.hostname==='panthorium-staging.onrender.com'&&(/^\/admin(?:\/|\.html)?$/.test(location.pathname));}
function sync(){
 ...
 if(stagingAdmin())window.PanthoriumStagingAdminDesktop?.sync?.();
}
function scheduleSync(){requestAnimationFrame(sync);}
window.addEventListener('panthorium:auth-changed',scheduleSync);
window.addEventListener('panthorium:apps-changed',scheduleSync);
window.addEventListener('panthorium:desktop-ready',scheduleSync);

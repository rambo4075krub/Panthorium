...
for (const script of ["branding.js", "phase2-auth.js", "user-manager.js", "security-dashboard.js", "ui-layout.js"]) {
...
app.get("/", (req, res, next) => {
  try {
    ...
    if (!html.includes('/branding.js')) html = html.replace(/<\/body>/i, '  <script src="/branding.js?v=phase3-ui-layout"></script>\n</body>');
    if (!html.includes('/phase2-auth.js')) html = html.replace(/<\/body>/i, '  <script src="/phase2-auth.js?v=phase3-ui-layout"></script>\n</body>');
    if (!html.includes('/user-manager.js')) html = html.replace(/<\/body>/i, '  <script src="/user-manager.js?v=phase3-ui-layout"></script>\n</body>');
    if (!html.includes('/security-dashboard.js')) html = html.replace(/<\/body>/i, '  <script src="/security-dashboard.js?v=phase3-ui-layout"></script>\n</body>');
    if (!html.includes('/ui-layout.js')) html = html.replace(/<\/body>/i, '  <script src="/ui-layout.js?v=phase3-ui-layout"></script>\n</body>');

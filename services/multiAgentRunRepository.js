class MultiAgentRunRepository {
  ...
  async init() {
    ...
    await this.pool.query('ALTER TABLE panthorium_multi_agent_runs ADD COLUMN IF NOT EXISTS provider TEXT');
  ...
  normalize(row) {
    if (!row) return null;
    return { orchestrationId: row.orchestration_id, userId: row.user_id, request: row.request, provider: row.provider || null, roles: row.roles || [], outputs: row.outputs || [], status: row.status, currentRole: row.current_role || null, workflowId: row.workflow_id || null, createdAt: row.created_at, updatedAt: row.updated_at };
  }
  async save(run) {
    const row = { orchestration_id: run.orchestrationId, user_id: run.userId, request: run.request, provider: run.provider || null, roles: run.roles || [], outputs: run.outputs || [], status: run.status, current_role: run.currentRole || null, workflow_id: run.workflowId || null, created_at: run.createdAt || new Date().toISOString(), updated_at: new Date().toISOString() };
    if (!this.pool) { this.memory.set(run.orchestrationId, row); return this.normalize(row); }
    const result = await this.pool.query(`INSERT INTO panthorium_multi_agent_runs(orchestration_id,user_id,request,provider,roles,outputs,status,current_role,workflow_id,created_at,updated_at)
      VALUES($1,$2,$3,$4,$5::jsonb,$6::jsonb,$7,$8,$9,$10,NOW())
      ON CONFLICT(orchestration_id) DO UPDATE SET provider=EXCLUDED.provider,roles=EXCLUDED.roles,outputs=EXCLUDED.outputs,status=EXCLUDED.status,current_role=EXCLUDED.current_role,workflow_id=EXCLUDED.workflow_id,updated_at=NOW() RETURNING *`,
      [row.orchestration_id,row.user_id,row.request,row.provider,JSON.stringify(row.roles),JSON.stringify(row.outputs),row.status,row.current_role,row.workflow_id,row.created_at]);

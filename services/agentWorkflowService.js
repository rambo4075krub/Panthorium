...
class AgentWorkflowService {
  constructor({ agentService, gateway, audit, ttlMs = 10 * 60 * 1000 } = {}) {
  ...
  parse(text, tools) {
    ...
    let data;
    try { data = JSON.parse(raw); } catch { return { ok: false, error: 'invalid_workflow_json' }; }
    if (!data || !Array.isArray(data.steps) || data.steps.length > 5) return { ok: false, error: 'invalid_workflow_steps' };
    const catalog = new Map(tools.map((t) => [t.id, t]));
    const steps = [];
    for (const input of data.steps) {
      const tool = catalog.get(input?.toolId);
      if (!tool) return { ok: false, error: 'invalid_workflow_tool' };
  ...
  cleanup() {
    const now = Date.now();
    for (const [id, wf] of this.pending.entries()) if (wf.expiresAt <= now) this.pending.delete(id);
  }

  async plan({ user, request, preferredProvider, requestId }) {
    const workflowId = randomUUID();
    const tools = this.catalog(user);
    const started = Date.now();
    this.audit?.record('agent.workflow_plan_started', { userId: user?.sub, requestId, workflowId, toolCount: tools.length });
    const result = await this.gateway.complete({
      systemPrompt: this.prompt(tools),
      history: [{ role: 'user', content: String(request || '').trim() }],
      preferredProvider,
      userId: user?.sub,
      sessionId: `agent-workflow:${workflowId}`
    });
    if (!result.ok) {
      this.audit?.record('agent.workflow_plan_failed', { userId: user?.sub, requestId, workflowId, error: result.error || 'planner_failed' });
      return { ok: false, workflowId, error: result.error || 'planner_failed' };
    }
    const parsed = this.parse(result.text, tools);
    if (!parsed.ok) {
      this.audit?.record('agent.workflow_plan_failed', { userId: user?.sub, requestId, workflowId, error: parsed.error });
      return { ok: false, workflowId, error: parsed.error };
    }
    ...
    return { ok: true, workflowId, workflow: parsed.workflow, provider: result.provider || null, model: result.model || null };
  }
  ...
  async run({ user, request, preferredProvider, requestId }) {
    ...
    const execution = await this.continueExecution(state, { requestId });
    return { ...planned, ...execution, workflow: planned.workflow };
  }

  async confirm({ user, workflowId, requestId }) {
    this.cleanup();
    const state = this.pending.get(String(workflowId || ''));
  ...
  cancel({ user, workflowId, requestId }) {
    this.cleanup();
    const state = this.pending.get(String(workflowId || ''));
    if (!state) return { ok: false, error: 'workflow_not_found' };
    if (state.user?.sub !== user?.sub) return { ok: false, error: 'workflow_permission_denied' };

...
class AgentWorkflowService {
  constructor({ agentService, gateway, audit, runs, ttlMs = 10 * 60 * 1000 } = {}) { this.agentService = agentService; this.gateway = gateway; this.audit = audit; this.runs = runs || null; this.ttlMs = ttlMs; this.pending = new Map(); }
  ...
  async continueExecution(state, { confirmed = false, requestId } = {}) {
    ...
    while (state.index < state.steps.length) {
      ...
      if (!validation.ok) { this.pending.delete(state.workflowId); await this.updateRun(state, { status: 'failed', error: validation.error, completedAt: new Date().toISOString() }); this.audit?.record('agent.workflow_failed', { userId: state.user?.sub, requestId, workflowId: state.workflowId, toolId: step.toolId, stepIndex: state.index, error: validation.error }); return { ok: false, workflowId: state.workflowId, executed: false, error: validation.error, results }; }
      if (step.requiresConfirmation && !confirmed) { state.results = results; state.expiresAt = Date.now() + this.ttlMs; this.pending.set(state.workflowId, state); await this.updateRun(state, { status: 'waiting_confirmation' }); return { ok: true, workflowId: state.workflowId, executed: false, confirmationRequired: true, pendingStep: { index: state.index, ...step }, results }; }
      ...
      if (!execution.ok) { this.pending.delete(state.workflowId); await this.updateRun(state, { status: 'failed', error: execution.error || 'tool_failed', completedAt: new Date().toISOString() }); this.audit?.record('agent.workflow_failed', { userId: state.user?.sub, requestId, workflowId: state.workflowId, toolId: step.toolId, stepIndex: state.index - 1, error: execution.error || 'tool_failed' }); return { ok: false, workflowId: state.workflowId, executed: false, error: execution.error || 'tool_failed', results }; }
    ...
    this.pending.delete(state.workflowId); this.audit?.record('agent.workflow_completed', { userId: state.user?.sub, requestId, workflowId: state.workflowId, steps: state.steps.length }); return { ok: true, workflowId: state.workflowId, executed: true, completed: true, results, answer: state.answer || null };
  }
  ...
  async confirm({ user, workflowId, requestId }) { await this.cleanup(); const state = this.pending.get(String(workflowId || '')); if (!state) return { ok: false, error: 'workflow_not_found' }; if (state.user?.sub !== user?.sub) return { ok: false, error: 'workflow_permission_denied' }; return this.continueExecution(state, { confirmed: true, requestId }); }
  async cancel({ user, workflowId, requestId }) { await this.cleanup(); const state = this.pending.get(String(workflowId || '')); if (!state) return { ok: false, error: 'workflow_not_found' }; if (state.user?.sub !== user?.sub) return { ok: false, error: 'workflow_permission_denied' }; this.pending.delete(state.workflowId); await this.updateRun(state, { status: 'cancelled', completedAt: new Date().toISOString() }); this.audit?.record('agent.workflow_cancelled', { userId: user?.sub, requestId, workflowId: state.workflowId, stepIndex: state.index }); return { ok: true, workflowId: state.workflowId, cancelled: true }; }
}

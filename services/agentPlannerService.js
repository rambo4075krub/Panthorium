...
class AgentPlannerService {
  ...
  toolCatalog(user) {
    return this.agentService.catalogFor(user).map((tool) => ({ id: tool.id, description: tool.description, permission: tool.permission || null, risk: tool.risk || 'low', mutates: !!tool.mutates, requiresConfirmation: !!tool.requiresConfirmation }));
  }
  plannerPrompt(tools) {

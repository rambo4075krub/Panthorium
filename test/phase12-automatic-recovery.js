...
(async()=>{
 ...
 const r=await recovery.recover(s.versionId);assert.equal(r.ok,true);assert.equal(created.length,2);assert(created.every(x=>x.tags.includes('automatic-recovery')));const updated=await repo.get(s.versionId);assert.equal(updated.metadata.recoveryAttempts,1);assert.equal(updated.metadata.recoveryCompletedAt != null,true);recovery.shutdown();console.log('Phase 12 automatic recovery tests passed');

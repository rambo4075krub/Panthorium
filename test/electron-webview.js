...
assert.match(preload,/panthoriumDesktop/); assert.match(external,/<webview/); assert.match(external,/isElectron/); assert.doesNotMatch(external,/<iframe/i); assert.match(sentinel,/recorderSupported/); assert.match(sentinel,/\/api\/speech\/transcribe/); assert.match(sentinel,/MediaRecorder/); assert.match(sentinel,/pt_server_release/); assert.match(sentinel,/cache:\s*"no-store"/);

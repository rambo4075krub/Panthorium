...
(async()=>{
 ...
 const admin={sub:'admin-1',permissions:['settings','sentinel:command','chat']};const created=await integrations.create({user:admin,name:'Webhook',endpointUrl:'https://hooks.example.com/task'});const integrationId=created.integration.integrationId;
 const sentinel={status(){return{};},providerCatalog(){return[];},async clearConversation(){}};const tools=new ToolRegistry({sentinel,integrations});const agent=new AgentService({tools,audit,policy:new AgentPolicyService()});
 ...
 const other={sub:'admin-2',permissions:['settings','sentinel:command','chat']};assert.equal((await integrations.history(other,{integrationId})).executions.length,0);
